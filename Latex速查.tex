...
\usepackage{amsmath} %数学调用
\usepackage{yhmath} %它提供了一组大型数学分界符，其中有花括号、圆括号、方括号、三角括号和重音号等；
\usepackage{amssymb} %数学符号调用，常用于数域
\usepackage{extarrows} %额外箭头表示
\usepackage{enumerate} %编号选项
\usepackage{makecell} % 表格内换行
\usepackage{paralist} %提供三个编号环境
\usepackage{fancyhdr} %页脚页眉分割线设置
\usepackage{datetime} %时间宏
\usepackage{graphicx} %照片插入
\usepackage{wrapfig} %提供了一个  wrapfigure 环境来排版小照片
\usepackage{fontspec} %选择字体
\usepackage{bm}       % 同时黑体斜体
\usepackage{listings} % 插入代码
\usepackage[all]{xy} 
\usepackage{esint} %积分符号
\usepackage{bigints} %更大的积分号
\usepackage{mathrsfs}  %常用于空间或者变换表示
\usepackage{tcolorbox} %\tcbuselibrary 来调用 tcolorbox 提供的丰富程序库
\usepackage{ulem} %提供各种各样的标记线uline
\usepackage{tikz} %绘画指南包
\usepackage{pifont}
...
%垂直线的宽度设置
%\columnsep 12pt  
%列间隔，实际默认间距差不多，故未启用，可自行调节
\columnseprule 1pt
\columnsep 8pt  

\special{dvipdfmx:config z 0}
%取消PDF压缩，加快速度，最终版本生成的时候最好把这句话注释掉
...
%本段允许跨页显示公式，调用的是amsmath%
%另外，在你想换页的地方使用命令\displaybreak，也带[n]，最好放在换行符\\ 之前。
%我们也可以在多行公式的行之间使用\\* 来阻止此处换行。
\allowdisplaybreaks
...
%分2栏，调用的是multitoc宏包
%双栏时\newpage是换到新的列！而\clearpage才是真正的换到新的页！另外单栏时没有区别，%   
\begin{multicols}{2}     
\begin{enumerate}
...
$ \gg $\q \verb|\gg| \\
$ \ll $\q \verb|\ll| \\
$ \equiv $\q \verb|\equiv| \\%恒等于
$ \sim $\q \verb|\sim| \\%approximately equal to大约等于
$ \backsim $\q \verb|\backsim| \\
$ \approx $\q \verb|\approx| \\%similar to近似于
$ \cong $\q \verb|\cong| \\%全等(三角形)
$ \pm $\q \verb|\pm| \\
$ \mp $\q \verb|\mp| \\
$ \in $\q \verb|\in| \\
